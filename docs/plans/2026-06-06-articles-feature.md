...
### 権限はサーバーのリアルタイム判定(URL に依存しない・確定)

可視性は記事 URL の秘匿性ではなく、アクセスごとの `isArticleVisibleTo()` で判定する。条項 4 の「旅行メンバー」も毎アクセスで `tripMembers` を都度照会するため、**旅行から外れる/紐づけが解除されると次回アクセスで 404** になる。URL が流出しても非メンバーは 404。「メンバーである間だけ見える」のは仕様どおりの帰結。

### 旅行内では「埋め込み表示」で独立 URL を露出しない(確定)

「private 記事の独立 URL が存在すること」自体への懸念に対し、API/可視性モデルは変更せず **UI 層**で対処する:
- 旅行詳細から記事を見るときは、独立ページ `/articles/:id` に遷移させず、旅行詳細内に**インライン/モーダルで埋め込み表示**する(`GET /api/trips/:id/articles` を使用)。これで旅行コンテキストから独立 URL を前面に出さない。
- 記事全体の一覧(`/articles`)やプロフィール公開記事は従来どおり独立ページで表示(可視性ロジックは全経路で共通)。
- 埋め込み方式への全面移行(記事詳細 API から条項 4 を外す案)は採らない。いいねの可視判定の捻れ・表示形態の二重化を避け、詳細/いいね/プロフィール公開を単一の可視性ロジックで一貫させるため。
...
- 旅行詳細に「関連記事」導線を追加(`GET /api/trips/:id/articles`)
  - PC: 旅行詳細の右サイドバーに「関連記事」ボタン/セクション
  - SP: 旅行詳細の 3 点リーダーメニュー内に「関連記事」項目
- アクセス導線は 2 系統を両立(確定): ①記事全体の一覧 = グローバルナビの独立タブ(ブックマークと同格)②その旅行の関連記事 = 旅行詳細から。記事は旅行に紐づけず private も作れるため、①の独立入口は必須
...
4. [x] フロント(一覧 / 作成 / 編集 / 詳細、PC/SP、タグ・いいね・紐づけ UI)
5. [x] プロフィール公開表示 + 旅行詳細の関連記事(埋め込み表示)
